...
if __name__ == "__main__":
    ...
    ##conexion.crearTablaFichaje()
    ##conexion.crearTablaNombreHabitaciones()
    ##conexion.crearTablaRecepcionista()
    ##conexion.crearTablaReservaActividad()
    ##conexion.crearTablaReservaHabitacion()
    ##conexion.crearTablaTiposReserva()
    error = 1
    opciones = 0
    incorrecta = 1
    ##unittest.main()
    while incorrecta !=0:
        while error!= 0:
            ...
            if rol == 1:
                codrecepcionista = int(input('''Ingrese su clave de seguridad'''))
                if codrecepcionista == 1234:
                    try:
                    ...
                    print("Pin incorecto. Intente nuevamente")

            elif rol == 2:
                codcliente = int(input('''Ingrese su clave de seguridad'''))
                if codcliente == 43211:
                    try:
                        cliente = int(input('''¿Que desea hacer?
                                    1- Reservar una habitacion
                                    2- Reservar una actividad
                                    3- Buscar habitaciones por filtro
                                    4- Buscar actividades por filtro
                                    5- Ver actividades
                                    6- Cancelar reserva habitacion
                                    7- Cancelar reserva actividades
                                    8- Ver mis datos
                                    9- Salir del sistema'''))
                    ...
                    if cliente == 1:
                        ...
                        ##reserva habitacion cancelar actividades
                        incorrecta = 0
